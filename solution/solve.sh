#!/bin/bash
# Oracle solution for terminal-bench / Harbor (temporary copy from GitHub).
# Copy to: <your-task>/solution/solve.sh — do not leave on the agent-facing repo.
...
if old not in text:
    raise SystemExit("chargeFailure.ts GATEWAY_TIMEOUT block not found; already patched?")
...
if guard not in ctext:
    raise SystemExit("checkout.ts guard anchor not found")
