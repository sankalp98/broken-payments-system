...
Temporary: copy into your Harbor task tests/ folder, then remove from the
agent-facing payments repo before running the benchmark.
"""
...
def test_reproduce_passes_without_multi_capture() -> None:
    proc = run(["npm", "run", "reproduce"], timeout=180)
    assert proc.returncode == 0, (
        "npm run reproduce must exit 0 after fix (no multi-capture orders):\n"
        f"stdout:\n{proc.stdout}\nstderr:\n{proc.stderr}"
    )


def test_retries_not_disabled() -> None:
    retry_policy = (REPO / "src" / "retryPolicy.ts").read_text()
    retry_worker = (REPO / "src" / "retryWorker.ts").read_text()
    checkout = (REPO / "src" / "checkout.ts").read_text()

    assert "RETRY_ENABLED" in retry_policy
    assert "true" in retry_policy.lower() or "RETRY_ENABLED = true" in retry_policy
    assert "processRetryQueue" in retry_worker or "runTrafficSpikeRetries" in retry_worker
    assert not re.search(r"RETRY_ENABLED\s*=\s*false", retry_policy)
    assert not re.search(r"maxInProcessAttempts\(\)\s*{\s*return\s*0", checkout)
